...
# 홈 페이지에 게시글 목록 불러옴
@app.get("/posts")
def posts_get(page: int=1, limit: int=2, category: str = None,
...
def post_get(data: sPostReq, 
    ...
    print(resp is RESULT_CODE.FAILED.value)
    if resp is RESULT_CODE.NOT_FOUND.value:
        raise HTTPException(status_code=404, detail="게시물이 없습니다.")
    if resp is RESULT_CODE.FAILED.value:
